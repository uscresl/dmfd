...
import warnings
from awac.utils.serialization_utils import convert_json
import wandb
...
class Logger:
    ...
    def __init__(self, output_dir=None, output_fname='progress.txt', exp_name=None, wandb_logging=None):
        """
        Initialize a Logger.
    ...
    def save_state(self, state_dict, itr=None, force_save=False):
        ...
        if hasattr(self, 'pytorch_saver_elements'):
            return self._pytorch_simple_save(itr)
...
class EpochLogger(Logger):
    ...
    def get_stats(self, key):
        ...
        v = self.epoch_dict[key]
        vals = np.concatenate(v) if isinstance(v[0], np.ndarray) and len(v[0].shape)>0 else v
        return self.statistics_scalar(vals)
