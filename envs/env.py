...
class SoftGymEnvSB3(gym.Env):
    def __init__(self, **config):
        self._env = SOFTGYM_ENVS[config.get('env')](**config.get('env_kwargs'))
        self._env = normalize(self._env, scale_reward=config.get('scale_reward'), clip_obs=config.get('clip_obs'))
        self.symbolic = config.get('symbolic')
        self._env.seed(config.get('seed'))
        self.max_episode_length = config.get('max_episode_length')
        self.action_repeat = config.get('action_repeat')
        ...
        self.obs_process = config.get('obs_process')
