...
class Logger(object):
    ...
    def log(self, key, value, step, n=1, log_frequency=1):
        ...
        if self.chester_log is not None:
            self.chester_log.record_tabular(key, value)
    ...
    def dump(self, step, save=True, ty=None):
        if len(self._eval_mg._prime_meters()) > 0 and self.chester_log is not None:
            self.chester_log.dump_tabular()
