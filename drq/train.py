...
def run_task(vv, log_dir=None, exp_name=None):
    ...
    with open("drq/config.yml", "r") as stream:
        try:
    ...
    if vv['is_eval']:
        if vv['eval_over_five_seeds']:
            workspace.eval_five_seeds()
        else:
            workspace.eval_gen_videos()
    else:
        workspace.run()
...
class Workspace(object):
    def __init__(self, cfg):
        ...
        self.checkpoint_path = cfg.checkpoint
    ...
    def eval_five_seeds(self):
        ...
        ckpt_file_path = self.checkpoint_path
    ...
    def eval_gen_videos(self):
        for episode in range(self.cfg.num_eval_episodes):
            ...
            episode_step = 0
            ep_info = []
            frames = [self.env.get_image(1024, 1024)]
            rewards = []
            ep_normalized_perf = []
            ...
            while not done:
                ...
                episode_step += 1
                ep_info.append(info)
                frames.append(self.env.get_image(1024, 1024))
                rewards.append(reward)
                ep_normalized_perf.append(info['normalized_performance'])
            save_numpy_as_gif(np.array(frames), os.path.join(self.video_dir, f'ep_{episode}_{ep_normalized_perf[-1]}.gif'))
    ...
    def evaluate(self):
        ...
        for key, val in get_info_stats(infos).items():
            ...
            if self.wandb:
                self.wandb_run.log({f'val/info_{key}': val, 'num_timesteps': self.step})
