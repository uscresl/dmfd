...
class PickerPickAndPlace(Picker):
    def __init__(self, num_picker, env=None, picker_low=None, picker_high=None, **kwargs):
        super().__init__(num_picker=num_picker,
                         picker_low=picker_low,
                         picker_high=picker_high,
                         **kwargs)
        picker_low, picker_high = list(picker_low), list(picker_high)
        total_num_actions = self.num_picker * 2
        # implementation with picker action [backup]
        # self.action_space = Box(np.array([*picker_low, 0.] * total_num_actions),
        #                         np.array([*picker_high, 1.] * total_num_actions), dtype=np.float32)
        self.action_space = Box(np.array([*picker_low] * total_num_actions),
                                np.array([*picker_high] * total_num_actions), dtype=np.float32)
        self.delta_move = 0.01
        self.env = env

    def step_helper(self, action, mode=None):
        # [:self.num_picker, :3] because we have an extra box in the environment. Default was [:, :3]
        curr_pos = np.array(pyflex.get_shape_states()).reshape(-1, 14)[:self.num_picker, :3]
        end_pos = np.vstack([self._apply_picker_boundary(picker_pos) for picker_pos in action[:, :3]])

        if mode == 'xz':
            # only move in x and z positions
            end_pos[:, 1] = curr_pos[:, 1]
    ...
    def step(self, action):
        ...
        action_pick = action[:self.num_picker]
        action_place = action[self.num_picker:]

        # add picker action
        action_pick = np.insert(action_pick, 3, 0, axis=1)
        action_place = np.insert(action_place, 3, 1, axis=1)

        ############### pick action ###############
        # pick action - step 1: move to pick x,z positions
        self.step_helper(action_pick, mode='xz')

        # pick action - step 2: move to pick y position (x,z positions do not change) and pick
        action_pick[:, 3] = 1
        self.step_helper(action_pick)

        ############### place action ###############
        # place action - step 1: move to place y position (x,z positions do not change)
        action_pick[:, 1] = action_place[:, 1]
        self.step_helper(action_pick)

        # place action - step 2: move to place x,z positions
        self.step_helper(action_place)

        # place action - step 3: drop
        action_place[:, 3] = 0
        self.step_helper(action_place)

    # Gautam: instead of moving to place location and drop. This version moves to a place slightly higher (y-axis)
    # than place location and then drop the cloth.
    # def step(self, action):
    #     action = action.reshape(-1, 3)
    #     action_pick = action[:self.num_picker]
    #     action_place = action[self.num_picker:]
    ...
    #     # pick action - step 1: move to pick x,z positions
    #     self.step_helper(action_pick, mode='xz')
    #     print('pick action - step 1: move to pick x,z positions')

    #     # pick action - step 2: move to pick y position (x,z positions do not change) and pick
    #     action_pick[:, 3] = 1
    #     self.step_helper(action_pick)
    #     print('pick action - step 2: move to pick y position (x,z positions do not change) and pick')

    #     ############### place action ###############
    #     # place action - step 1: move to place y position+0.1 (x,z positions do not change)]
    #     action_pick[:, 1] = action_place[:, 1] + 0.1
    #     self.step_helper(action_pick)
    #     print('place action - step 1: move to place y position+0.1 (x,z positions do not change)]')

    #     # place action - step 2: move to place x,z positions. Constant y position
    #     action_pick = action_place.copy()
    #     action_pick[:, 1] += 0.1
    #     self.step_helper(action_pick)
    #     print('place action - step 2: move to place x,z positions')

    #     # place action - step 3: move to place y position
    #     self.step_helper(action_place)
    #     print('place action - step 3: move to place y positions')

    #     for i in range(100):
    #         pyflex.step()
    #         self.env.render(mode='rgb_array')
    #     print(f'place action - waited for 100 steps')

    #     # place action - step 4: drop
    #     action_place[:, 3] = 0
    #     self.step_helper(action_place)
    #     print('place action - step 4: drop')
    ...
    #     picker_act_pick = action_pick[:, 3].copy()
    #     picker_act_place = action_place[:, 3].copy()
    ...
    #     # pick action - step 2: move to pick y position (x,z positions do not change)
    #     action_pick[:, 3] = picker_act_pick # restore original picker action for pick
    #     self.step_helper(action_pick)
    ...
    #     # place action - step 2: move to place x,z positions
    #     action_place[:, 3] = picker_act_pick # during this phase, place's picker action should come from pick's
    #     self.step_helper(action_place)

    #     # place action - step 3: perform place's picker action
    #     action_place[:, 3] = picker_act_place # use place's picker action
    #     self.step_helper(action_place)
