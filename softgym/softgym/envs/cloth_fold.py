...
class ClothFoldEnv(ClothEnv):
    ...
    def _reset(self):
        ...
        corner1_idx  = np.where(particle_pos[:, 0] == self.x_min)[0][0]
        corner2_idx  = np.where(particle_pos[:, 0] == self.x_max)[0][0]
        corner3_idx  = np.where(particle_pos[:, 2] == self.z_min)[0][0]
        corner4_idx  = np.where(particle_pos[:, 2] == self.z_max)[0][0]

        corner1 = particle_pos[corner1_idx][:3]
        corner2 = particle_pos[corner2_idx][:3]
        corner3 = particle_pos[corner3_idx][:3]
        corner4 = particle_pos[corner4_idx][:3]
        ...
        if corner1_idx in self.fold_group_a:
            self.corner_pick.append(corner1)
            self.corner_pick_idx.append(corner1_idx)
        else:
            self.corner_drop.append(corner1)
            self.corner_drop_idx.append(corner1_idx)

        if corner2_idx in self.fold_group_a:
            self.corner_pick.append(corner2)
            self.corner_pick_idx.append(corner2_idx)
        else:
            self.corner_drop.append(corner2)
            self.corner_drop_idx.append(corner2_idx)

        if corner3_idx in self.fold_group_a:
            self.corner_pick.append(corner3)
            self.corner_pick_idx.append(corner3_idx)
        else:
            self.corner_drop.append(corner3)
            self.corner_drop_idx.append(corner3_idx)

        if corner4_idx in self.fold_group_a:
            self.corner_pick.append(corner4)
            self.corner_pick_idx.append(corner4_idx)
        else:
            self.corner_drop.append(corner4)
            self.corner_drop_idx.append(corner4_idx)

        # calculate distance between corner_pick[0] and corner_drop[0], corner_drop[1]
        dist0 = np.linalg.norm(self.corner_pick[0] - self.corner_drop[0])
        dist1 = np.linalg.norm(self.corner_pick[0] - self.corner_drop[1])
