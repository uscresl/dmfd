...
def run_task(vv, log_dir, exp_name):
    ...
    # Run policy
    initial_states, state_trajs, action_trajs, ob_trajs, ob_img_trajs, configs, all_infos = [], [], [], [], [], [], []
    for i in range(vv['test_episodes']):
        ...
        for j in range(env.horizon):
            ...
            ob_img_traj.append(env.get_image(128, 128))
        ...
        ob_img_trajs.append(ob_img_traj.copy())
...
def main():
    ...
    # Experiment
    parser.add_argument('--exp_name', default='cem_ropeflatten', type=str)
    parser.add_argument('--env_name', default='RopeFlatten')
    parser.add_argument('--log_dir', default='./data/cem_ropeflatten')
    parser.add_argument('--test_episodes', default=100, type=int)
    parser.add_argument('--seed', default=100, type=int)

    # CEM max_iters:2 and timestep_per_decision:11000 combination has ~90% perf on ropeflatten when env_kwargs_num_variations=1000
    # CEM max_iters:2 and timestep_per_decision:1000 combination has ~95% perf on ropeflatten when env_kwargs_num_variations=1
    parser.add_argument('--max_iters', default=2, type=int) # default 10
    parser.add_argument('--timestep_per_decision', default=1000, type=int) # default 21000
    ...
    parser.add_argument('--env_kwargs_num_variations', default=1, type=int) # default is 1000

    args = parser.parse_args()
    args.log_dir = f'{args.log_dir}-{args.test_episodes}eps-{args.seed}-numenv{args.env_kwargs_num_variations}'
