from asym_sac.train import run_task
...
def main():
    ...
    # Experiment
    parser.add_argument('--name', default='Asym_SAC_ROPE', type=str)
    parser.add_argument('--env_name', default='RopeFlatten')
    parser.add_argument('--log_dir', default='./data/asym_sac_ropeflatten/')
    ...
    # Asymmetric SAC with prefilled replay buffer
    parser.add_argument('--expert_data', default=None, type=str, help="Fill up the expert replay buffer with expert demonstrations")

    # Pretrained BC
    parser.add_argument('--bc_checkpoint', default=None, type=str, help="pretrained BC checkpoint file to initialize SAC actor")
    ...
    parser.add_argument('--env_kwargs_observation_mode', default='cam_rgb_key_point', type=str)  # Should be in ['key_point', 'cam_rgb', 'point_cloud']
    ...
    args.name = f'{env_name}_CoL_{now}' if not args.name else args.name
